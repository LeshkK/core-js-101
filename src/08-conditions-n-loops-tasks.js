...
function doRectanglesOverlap(rect1, rect2) {
  const a = [rect1.rightBot];
  const b = [rect2.rightBot];
  const c = [rect1.len];
  const d = [rect2.len];
  a[0] = rect1.top + rect1.height;
  b[0] = rect2.top + rect2.height;
  c[0] = rect1.left + rect1.width;
  d[0] = rect2.left + rect2.width;
  if (rect1.left < d[0] && c[0] > rect2.left) {
    if (rect1.top < b[0] && a[0] > rect2.top) {
      return true;
    }
    return false;
  }
...
function getDigitalRoot(num) {
  const a = num.toString(10).replace(/\D/g, '0').split('').map(Number);
  const sum = [0];
  for (let i = 0; i < a.length; i += 1) {
    sum[0] += a[i];
  }
  if (sum[0].toString(10).replace(/\D/g, '0').split('').map(Number).length === 2) {
    const b = sum[0].toString(10).replace(/\D/g, '0').split('').map(Number);
    sum[0] = 0;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    ...
    if (count[0] === pathes.length) { result[0] += char; }
  }
  return pathes;
}
